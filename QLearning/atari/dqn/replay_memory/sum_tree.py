...
class SumTree(object):
    ...
    def set_delta(self, node_index, value):
        ...
        if value < 0.0:
            raise ValueError('Sum tree values should be nonnegative. Got {}'.
                             format(value))
        self.max_recorded_priority = float(max(value, self.max_recorded_priority))
    ...
    def set(self, node_index, value):
        ...
        for nodes_at_this_depth in reversed(self.nodes[1:]):
            nodes_at_this_depth[node_index] = tally
            left = (node_index // 2) * 2
            right = (node_index // 2) * 2 + 1
            tally = nodes_at_this_depth[left] + nodes_at_this_depth[right]
            node_index //= 2

        self.nodes[0][0] = tally
